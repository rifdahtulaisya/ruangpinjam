...
@section('content')
                        ...
                        <div id="statusOptions"
                            ...
                            <div class="py-2">
                                ...
                                <div class="option flex items-center gap-3 px-4 py-2.5 hover:bg-yellow-50 cursor-pointer"
                                    data-value="menunggu" data-label="Menunggu Konfirmasi">
                                    <i class="fas fa-clock text-yellow-500 w-4"></i> Menunggu Konfirmasi
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-gradient-to-r from-yellow-50 to-yellow-100 border border-yellow-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-yellow-800 font-medium">Menunggu</p>
                        <p class="text-2xl font-bold text-yellow-900 mt-1">{{ $counts['menunggu'] ?? 0 }}</p>
                        <p class="text-xs text-yellow-600 mt-1">Butuh konfirmasi</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-red-50 to-red-100 border border-red-200 rounded-2xl p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-red-800 font-medium">Ditolak</p>
                        <p class="text-2xl font-bold text-red-900 mt-1">{{ $counts['ditolak'] ?? 0 }}</p>
                            ...
                            <tr class="hover:bg-slate-50/50 transition">
                                <td class="py-4 px-6">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-medium text-slate-800">{{ $peminjaman->user->name ?? 'Tidak diketahui' }}</div>
                                            <div class="text-xs text-slate-500">{{ $peminjaman->user->email ?? '-' }}</div>
                                            <div class="text-xs text-slate-500">Kelas: {{ $peminjaman->user->kelas ?? '-' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-4 px-6">
                                ...
                                <td class="py-4 px-6">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-sm text-slate-700">{{ $peminjaman->tanggal_peminjaman->format('d/m/Y') }}</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-calendar-check text-green-500 text-xs"></i>
                                            <span class="text-sm text-slate-700">{{ $peminjaman->tanggal_pengembalian->format('d/m/Y') }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-4 px-6">
                                    ...
                                    @endphp
                                    <span class="px-3 py-1 rounded-full text-sm font-medium border flex items-center gap-1 w-fit {{ $statusColors[$peminjaman->status] }}">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        @if($peminjaman->status == 'menunggu')
                                            <button onclick="showKonfirmasiModal({{ $peminjaman->id }}, 'setuju')"
                                            ...
                                            <button onclick="showKonfirmasiModal({{ $peminjaman->id }}, 'tolak')"
    ...
    <!-- Tombol Konfirmasi untuk pengembalian mandiri yang menunggu -->
...
@elseif($peminjaman->status == 'dipinjam')
    <!-- Tombol Kembalikan hanya untuk metode langsung -->
    @if($peminjaman->metode_pengembalian == 'langsung')
...
@endif
                        ...
                        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
                            ...
                            <div class="flex justify-between items-start mb-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-bold text-slate-800">{{ $peminjaman->user->name ?? 'Tidak diketahui' }}</div>
                                ...
                                @endphp
                                <span class="px-2 py-1 rounded-full text-xs font-medium border {{ $statusColors[$peminjaman->status] }}">
                                    {{ ucfirst($peminjaman->status) }}
                                </span>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <div class="flex items-center gap-1">
                                            <i class="fas fa-calendar-alt text-blue-500 text-xs"></i>
                                            <span class="text-sm">{{ $peminjaman->tanggal_peminjaman->format('d/m/Y') }}</span>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div class="flex items-center gap-1">
                                            <i class="fas fa-calendar-check text-green-500 text-xs"></i>
                                            <span class="text-sm">{{ $peminjaman->tanggal_pengembalian->format('d/m/Y') }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
                                    ...
                                    @if($peminjaman->status == 'dipinjam')
                                        <button onclick="{{ $peminjaman->metode_pengembalian == 'mandiri' ? "showVerifikasiModal({$peminjaman->id})" : "konfirmasiLangsungKembali({$peminjaman->id})" }}"
                                            class="flex-1 px-3 py-2 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition text-sm">
                                            <i class="fas fa-check mr-1"></i>
                                            {{ $peminjaman->metode_pengembalian == 'mandiri' ? 'Verifikasi' : 'Kembalikan' }}
                                        </button>
                                        @if($peminjaman->metode_pengembalian == 'mandiri')
                                            <button onclick="showTeguranModal({{ $peminjaman->id }})"
                                                class="flex-1 px-3 py-2 bg-orange-50 text-orange-700 rounded-lg hover:bg-orange-100 transition text-sm">
                                                <i class="fas fa-exclamation-triangle mr-1"></i> Teguran
                                            </button>
                                        @endif
                                    @endif
                                </div>
    ...
    <div id="detailModal" class="fixed inset-0 bg-black/50 z-[60] hidden items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl animate-slide-up max-h-[90vh] overflow-hidden flex flex-col">
            <!-- Header Modal -->
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl animate-slide-up">
            <div class="p-6">
                ...
                <div class="mb-6">
                    ...
                    <div id="noFotoMessage" class="hidden text-center p-8 border-2 border-dashed border-red-200 rounded-xl">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="flex items-center gap-3 p-4 border-2 border-red-200 rounded-xl cursor-pointer hover:bg-red-50 transition">
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-slide-up">
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <select id="alasanTeguran" class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-orange-500 focus:ring-3 focus:ring-orange-500/30 transition mb-3"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl animate-slide-up">
            <div class="p-6">
                ...
                <div class="rounded-xl overflow-hidden border-2 border-slate-200">
                    <img id="fotoModalImage" src="" alt="Foto bukti" class="w-full h-auto max-h-[70vh] object-contain">
                </div>
            ...
            <div class="p-6">
                ...
                <form id="exportForm">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:border-emerald-500 focus:ring-3 focus:ring-emerald-500/30 transition">
...
function showKonfirmasiPengembalianModal(peminjamanId) {
    Swal.fire({
        title: 'Konfirmasi Pengembalian',
        text: 'Apakah alat sudah dikembalikan dengan baik? Stok akan otomatis bertambah.',
        icon: 'question',
    ...
    function showDetailModal(peminjamanId) {
        ...
        function showKonfirmasiModal(peminjamanId, action) {
            ...
            if (action === 'setuju') {
                ...
                actionBtn.className = 'flex-1 px-4 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition';
            } else {
        ...
        function closeKonfirmasiModal() { closeModal('konfirmasiModal'); }
        ...
        function closeVerifikasiModal() { closeModal('verifikasiModal'); }
        ...
        function closeTeguranModal() { closeModal('teguranModal'); }
        ...
        function closeFotoModal() { closeModal('fotoModal'); }
        ...
        // Action Functions dengan route yang benar
